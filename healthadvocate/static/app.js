...
const HA = {
  ...
  renderSymptoms(data, el) {
    ...
    if (data.recommendations?.length) {
      html += `<div class="result-section"><h3>Recommendations</h3><ol>`;
      for (const r of data.recommendations) html += `<li class="result-text">${this.escapeHtml(r)}</li>`;
      html += `</ol></div>`;
    }
    el.innerHTML = html;
  },
  ...
  renderDocument(data, el) {
    let html = `<div class="result-section"><h3>Summary</h3><p class="result-text">${this.escapeHtml(data.summary)}</p></div>`;
    if (data.entities?.length) {
      html += `<div class="result-section"><h3>Medical Entities</h3><div class="entity-list">`;
      for (const e of data.entities) html += `<span class="entity-chip ${e.category}" title="${this.escapeHtml(e.explanation)}">${this.escapeHtml(e.text)} <small>(${e.label})</small></span>`;
      html += `</div></div>`;
      html += `<div class="result-section"><h3>Explanations</h3>`;
      for (const e of data.entities) html += `<div class="condition-item"><strong>${this.escapeHtml(e.text)}</strong>: ${this.escapeHtml(e.explanation)}</div>`;
      html += `</div>`;
    }
    if (data.pii_found?.length) {
      html += `<div class="result-section"><h3>Personal Information Detected</h3>`;
      for (const p of data.pii_found) html += `<div class="flag-item flag-warning">Found ${p.label}: "${this.escapeHtml(p.text)}"</div>`;
      html += `</div>`;
    }
    el.innerHTML = html;
  },
  ...
  renderBill(data, el) {
    let html = '';
    if (data.total) html += `<div class="result-section"><h3>Total</h3><div style="font-size:30px;font-weight:800;letter-spacing:-0.02em">${this.escapeHtml(data.total)}</div></div>`;
    if (data.line_items?.length) {
      html += `<div class="result-section"><h3>Line Items</h3>`;
      for (const item of data.line_items) {
        const amt = item.amount ? `$${item.amount.toFixed(2)}` : 'N/A';
        html += `<div class="condition-item"><span class="condition-name">${this.escapeHtml(item.description)}</span>${item.cpt_code ? `<small> (CPT: ${item.cpt_code})</small>` : ''}<span style="float:right;font-weight:600">${amt}</span></div>`;
      }
      html += `</div>`;
    }
    if (data.flags?.length) {
      html += `<div class="result-section"><h3>Flags</h3>`;
      for (const f of data.flags) html += `<div class="flag-item ${f.severity === 'warning' ? 'flag-warning' : 'flag-info'}">${this.escapeHtml(f.detail)}</div>`;
      html += `</div>`;
    }
    if (!data.line_items?.length && !data.flags?.length) html = '<p class="result-text">No bill items or issues detected. Try pasting a more detailed bill.</p>';
    el.innerHTML = html;
  },
  ...
  renderDenial(data, el) {
    let html = `<div class="result-section"><h3>Denial Analysis</h3>
      <p class="result-text"><strong>Type:</strong> ${this.escapeHtml(data.denial_type)}</p>
      <p class="result-text">${this.escapeHtml(data.denial_reason)}</p></div>`;
    if (data.key_arguments?.length) {
      html += `<div class="result-section"><h3>Key Arguments for Your Appeal</h3><ol>`;
      for (const a of data.key_arguments) html += `<li class="result-text">${this.escapeHtml(a)}</li>`;
      html += `</ol></div>`;
    }
    if (data.appeal_letter) html += `<div class="result-section"><h3>Generated Appeal Letter</h3><div class="appeal-letter">${this.escapeHtml(data.appeal_letter)}</div></div>`;
    el.innerHTML = html;
  },
  ...
  renderDischarge(data, el) {
    let html = '';
    if (data.plain_language) html += `<div class="result-section"><h3>Plain Language Translation</h3><div class="plain-language">${this.escapeHtml(data.plain_language)}</div></div>`;
    if (data.medications?.length) {
      html += `<div class="result-section"><h3>Medications</h3><div class="entity-list">`;
      for (const m of data.medications) html += `<span class="entity-chip drug">${this.escapeHtml(m.name)}</span>`;
      html += `</div></div>`;
    }
    if (data.follow_up?.length) {
      html += `<div class="result-section"><h3>Follow-Up</h3><ul class="followup-list">`;
      for (const f of data.follow_up) html += `<li>${this.escapeHtml(f)}</li>`;
      html += `</ul></div>`;
    }
    if (data.warnings?.length) {
      html += `<div class="result-section"><h3>Warnings</h3><ul class="warning-list">`;
      for (const w of data.warnings) html += `<li>${this.escapeHtml(w)}</li>`;
      html += `</ul></div>`;
    }
    if (data.entities_explained?.length) {
      html += `<div class="result-section"><h3>Terms Explained</h3>`;
      for (const e of data.entities_explained) html += `<div class="condition-item"><strong>${this.escapeHtml(e.term)}</strong>: ${this.escapeHtml(e.explanation)}</div>`;
      html += `</div>`;
    }
    el.innerHTML = html;
  },
  ...
  renderSecondOpinion(data, el) {
    let html = `<div class="result-section"><h3>Summary for Second Opinion</h3><p class="result-text">${this.escapeHtml(data.deidentified_summary)}</p></div>`;
    if (data.conditions?.length) {
    ...
    if (data.questions_for_specialist?.length) {
      html += `<div class="result-section"><h3>Questions for the Specialist</h3><ol>`;
      for (const q of data.questions_for_specialist) html += `<li class="result-text">${this.escapeHtml(q)}</li>`;
      html += `</ol></div>`;
    }
  ...
  renderCommunity(data, el) {
    ...
    if (data.health_alerts?.length) {
      html += `<div class="result-section"><h3>Health Alerts</h3>`;
      for (const a of data.health_alerts) html += `<div class="flag-item flag-danger">${this.escapeHtml(a)}</div>`;
      html += `</div>`;
    }
    if (data.affected_conditions?.length) {
      html += `<div class="result-section"><h3>Affected Conditions</h3><div class="entity-list">`;
      for (const c of data.affected_conditions) html += `<span class="entity-chip disease">${this.escapeHtml(c.name)}</span>`;
      html += `</div></div>`;
    }
    if (data.mentioned_treatments?.length) {
      html += `<div class="result-section"><h3>Mentioned Treatments</h3><div class="entity-list">`;
      for (const t of data.mentioned_treatments) html += `<span class="entity-chip drug">${this.escapeHtml(t.name)}</span>`;
      html += `</div></div>`;
    }
    if (data.findings?.length) {
      html += `<div class="result-section"><h3>Findings</h3>`;
      for (const f of data.findings) html += `<div class="condition-item"><span class="condition-name">${this.escapeHtml(f.entity)}</span><small> (${f.type})</small><div class="profile-detail"><em>${this.escapeHtml(f.context)}</em></div></div>`;
      html += `</div>`;
    }
    ...
    for (const p of profiles) {
      ...
      html += `<div class="profile-card">
        ...
        <div style="margin-top:10px">
          <input type="text" id="cond-${p.id}" placeholder="Add condition" style="width:45%;font-size:13px">
          <button class="btn-ghost btn-sm" onclick="HA.addCondition('${p.id}')">Add</button>
        </div>
      </div>`;
    }
    el.innerHTML = html;
    ...
    if (data.tracks?.length) {
      for (const t of data.tracks) {
        html += `<div class="track-item">
          <div><span class="track-concern">${this.escapeHtml(t.concern)}</span><small style="color:var(--text-3);margin-left:6px">${t.category}</small></div>
          <div style="display:flex;align-items:center;gap:6px">
            <span class="track-status ${t.status}">${t.status}</span>
            ${t.status !== 'resolved' ? `<button class="btn-ghost btn-sm" onclick="HA.updateTrackStatus('${t.id}','resolved')">Resolve</button>` : ''}
            ${t.status === 'active' ? `<button class="btn-ghost btn-sm" onclick="HA.updateTrackStatus('${t.id}','monitoring')">Monitor</button>` : ''}
          </div>
        </div>`;
      }
    } else {
